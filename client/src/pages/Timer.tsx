...
const TimerPage: React.FC = () => {
      ...
      try {
        ...
        if (tablesResponse.data.success) {
          const existingTable = tablesResponse.data.data.find(
            (table: any) =>
              table.table_number === newTimer.table_number ||
              table.table_code === newTimer.table_number
          );
          if (existingTable) {
  ...
  const getActiveTimersCount = () => {
    return timers.filter(
      (timer) => timer.status === "active" || timer.status === "overtime"
    ).length;
  };
              ...
              <div key={timer.id} className={`timer-card ${status}`}>
                  ...
                  <div className="progress-bar">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          100,
                          (timer.duration_minutes / TIME_LIMIT_MINUTES) * 100
                        )}%`,
                ...
                <div className="timer-controls">
                  ...
                  {timer.status === "completed" && (
                    <div className="completed-message">Session Completed</div>
                  )}
                </div>
              </div>
            );
        ...
        <div className="modal-overlay">
          <div className="add-timer-modal">
            ...
            <div className="modal-footer">
              ...
              <button className="confirm-btn" onClick={addNewTimer}>
                Start Timer
              </button>
            </div>
          </div>
